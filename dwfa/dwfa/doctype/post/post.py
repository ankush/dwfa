...
class Post(WebsiteGenerator):
	...
	def reset_featured_for_other_blogs(self):
		all_posts = frappe.get_all("Post", {"featured": 1})
		for post in all_posts:
			frappe.db.set_value("Post", post.name, "featured", 0)

	def fetch_cta(self):
		if frappe.db.get_single_value("Settings", "show_cta_in_blog", cache=True):
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and frappe.db.get_single_value(
			"Settings", "show_cta_in_blog", cache=True
		)
		...
		context.category = frappe.db.get_value(
			"Category", context.doc.category, ["title", "route"], as_dict=1
		)
		context.parents = [
			{"name": _("Home"), "route": "/"},
		...
		context.guest_allowed = frappe.db.get_single_value(
			"Settings", "allow_guest_to_comment"
		)

	def fetch_social_links_info(self):
		if not frappe.db.get_single_value("Settings", "enable_social_sharing", cache=True):
			return []
	...
	def make_route(self):
		if not self.route:
			return (
				frappe.db.get_value("Category", self.category, "route")
				+ "/"
...
def get_list_context(context=None):
	...
	settings = frappe.get_doc("Settings").as_dict(no_default_fields=True)
	list_context.update(settings)

	category_name = frappe.utils.escape_html(
		frappe.local.form_dict.category or frappe.local.form_dict.category
	)
	if category_name:
		category = frappe.get_doc("Category", category_name)
		list_context.blog_introduction = category.description or _(
			"Posts filed under {0}"
	...
	if settings.browse_by_category:
...
def get_blog_categories():
	...
	post, category = frappe.qb.DocType("Post"), frappe.qb.DocType("Category")
	return (
...
def clear_blog_cache():
	for blog in frappe.db.sql_list(
		"""select route from
		`tabPost` where ifnull(published,0)=1"""
	):
		clear_cache(blog)
...
def get_attachment(path):
	...
	return Response(wrap_file(frappe.local.request.environ, f), direct_passthrough=True)
...
def get_blog_list(
	...
	if filters and filters.get("blogger"):
		conditions.append(f"t1.blogger={filters['blogger']}")

	if category:
		conditions.append("t1.category=%s" % frappe.db.escape(category))
	...
	query = f"""\
		select
			t1.title, t1.name, t1.category, t1.route, t1.published_on, t1.read_time,
				...
				(select count(name) from `tabComment`
					where
						...
						and reference_doctype='Post'
						and reference_name=t1.name) as comments
		from `tab{doctype}` t1, `tabBlogger` t2
	...
	for post in posts:
		...
		post.category = frappe.db.get_value(
			"Category", post.category, ["name", "route", "title"], as_dict=True
		)
